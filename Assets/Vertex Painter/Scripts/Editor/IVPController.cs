...
namespace Ikari.VertexPainter
{
    public class IVPController : EditorWindow
    {
        ...
        void GetEditorData()
        {
            ...
            if (IVPData == null)
            {
                AssetDatabase.CreateAsset(ScriptableObject.CreateInstance<IVPData>(), editorDataPath);
        ...
        void DrawMenu()
        {
            ...
            menuRect = new Rect(menuRect.x, menuRect.y + 115, position.width + 2, 20);
                                ...
                                if (objectData.editorData.foldout)
                                {
                                    ...
                                    if (!Selection.transforms[i].GetComponent<IVPObjectData>().editorData.showingVertex)
                                    {
                                        if (GUILayout.Button("Show Vertex Colors"))
                                        {
                                            ...
                                            Undo.RecordObjects(new Object[] { Selection.transforms[i].GetComponent<Renderer>(), objectData }, "Show Vertex Color");
                                        ...
                                        if (GUILayout.Button("Show Normal"))
                                        {
                                            ...
                                            Undo.RecordObjects(new Object[] { Selection.transforms[i].GetComponent<Renderer>(), objectData }, "Show Normal Color");
                                    ...
                                    if (Selection.transforms[i].GetComponent<IVPObjectData>().editorData.showingWireframe)
                                    {
                                        ...
                                        {
                                            Undo.RecordObjects(new Object[] { Selection.transforms[i].GetComponent<Renderer>(), objectData }, "Show Wireframe");
            ...
            if (menuOptionsIndex == 0)
            {
                ...
                if (IVPData.Foldouts.tool)
                {
                    ...
                    if (IVPData.paintMode == PaintMode.Brush)
                    {
                        ...
                        IVPData.brushSize = IVPInterface.DrawSlider(IVPData.brushSize, 0.1f, 1, "Brush Size");

                        //Brush Strenght property
                        IVPData.brushStrenght = IVPInterface.DrawFloatField("Brush Strenght", IVPData.brushStrenght);
                        IVPData.brushStrenght = IVPInterface.DrawSlider(IVPData.brushStrenght, 0f, 1f, "Brush Strenght");
